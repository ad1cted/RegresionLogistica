...
df = df[["tiene_mora","salario","nivel_educacion"]]
...
x_train, x_test, y_train, y_test = train_test_split(df,clase, test_size=0.2, random_state = 0)
...
logistic_regresion =  LogisticRegression(max_iter=10000, n_jobs=-1, class_weight=[.1,.9])
...
## odds ratio
odds_ratio = pd.DataFrame(np.exp(logistic_regresion.coef_[0]))
...
#print (logistic_regresion.predict_proba(x_test))
